...
<div class="card">
  <div class="card-header">
    <strong>${uiLabelMap.BritShoppingLists}</strong>
    ...
    <div class="card-body">
        ...
        <#else>
          <label class="mb-2">${uiLabelMap.BritNoShoppingListsCreate}.</label>
          <form id="createEmptyShoppingList" action="<@ofbizUrl>createEmptyShoppingList</@ofbizUrl>" method="post">
             <input type="hidden" name="productStoreId" value="${productStoreId!}" />
...
<div class="card">
  <div class="card-header">
    <strong>${uiLabelMap.BritShoppingListDetail} - ${shoppingList.listName}</strong>
    ...
    <div class="card-body">
      ...
      <form name="updateList" method="post" action="<@ofbizUrl>updateShoppingList</@ofbizUrl>">
          ...
          <input type="hidden" class="inputBox" name="partyId" value="${shoppingList.partyId?if_exists}" />
            <div class="row">
              <div class="col-sm-6">
              <label for="listName">${uiLabelMap.BritListName}</label>
            ...
            <div class="row">
              <div class="col-sm-6">
              <label for="isPublic">${uiLabelMap.BritPublic}?</label>
            ...
            <div class="row">
              <div class="col-sm-6">
              <label for="isActive">${uiLabelMap.BritActive}?</label>
            ...
            <div class="row">
              <div class="col-sm-6">
              <label for="parentShoppingListId">${uiLabelMap.BritParentList}</label>
              <select name="parentShoppingListId" id="parentShoppingListId" class="form-control custom-select">
                ...
                <option value="">${uiLabelMap.BritNoParent}</option>
...
<div class="card">
    <div class="card-header">
        <strong>
            ${uiLabelMap.BritShoppingListReorder} - ${shoppingList.listName}
            <#if "N" == shoppingList.isActive?default("N")>
                ${uiLabelMap.BritOrderNotActive}
            </#if>
        </strong>
    ...
    <div class="card-body">
        <form name="reorderinfo" method="post" action="<@ofbizUrl>updateShoppingList</@ofbizUrl>">
            <input type="hidden" name="shoppingListId" value="${shoppingList.shoppingListId}" />
                <label>${uiLabelMap.BritRecurrence}</label>
          ...
          <div class="row">
            ...
            <select name="intervalNumber" class="custom-select mt-2">
                  <option value="">${uiLabelMap.BritSelectInterval}</option>
                  <option value="1" <#if (recurrenceRule.intervalNumber)?default(0) == 1>selected="selected"</#if>>${uiLabelMap.BritEveryDay}</option>
                  <option value="2" <#if (recurrenceRule.intervalNumber)?default(0) == 2>selected="selected"</#if>>${uiLabelMap.BritEveryOther}</option>
                  <option value="3" <#if (recurrenceRule.intervalNumber)?default(0) == 3>selected="selected"</#if>>${uiLabelMap.BritEvery3rd}</option>
                  <option value="6" <#if (recurrenceRule.intervalNumber)?default(0) == 6>selected="selected"</#if>>${uiLabelMap.BritEvery6th}</option>
                  <option value="9" <#if (recurrenceRule.intervalNumber)?default(0) == 9>selected="selected"</#if>>${uiLabelMap.BritEvery9th}</option>
                </select>
            </div>
          </div>
          <div class="row">
            ...
            <select name="frequency" class="custom-select my-2">
                  <option value="">${uiLabelMap.BritSelectFrequency}</option>
          ...
          <div class="row mt-3">
            <div class="col-sm-12">
                ...
                <a href="<@ofbizUrl>editcreditcard?DONE_PAGE=editShoppingList</@ofbizUrl>" class="btn btn-outline-secondary">${uiLabelMap.BritNewCreditCard}</a>
                <a href="<@ofbizUrl>editeftaccount?DONE_PAGE=editShoppingList</@ofbizUrl>" class="btn btn-outline-secondary">${uiLabelMap.BritNewEFTAccount}</a>
            </div>
          </div>
                  ...
                  <#if lastSlOrderDate?has_content>
                    ...
                    <div class="tabletext">
                      <table>
                        ...
                        <tr>
                          <td>${uiLabelMap.BritEstimateNextOrderDate}</td>
                          <td>:</td>
                          <td>${nextTimeString?default("${uiLabelMap.BritNotYetKnown}")}</td>
                        </tr>
                      </table>
                    </div>
                  </tr>
...
<div class="card">
    <div class="card-header">
      <strong>${uiLabelMap.BritChildShoppingList} - ${shoppingList.listName}</strong>
      <a href="<@ofbizUrl>addListToCart?shoppingListId=${shoppingList.shoppingListId}&amp;includeChild=yes</@ofbizUrl>" class="float-right">${uiLabelMap.BritAddChildListsToCart}</a>
    </div>
    <div class="card-body">
        <table class="table">
          <thead>
              <tr>
                <th>${uiLabelMap.BritListName}</th>
                <th>${uiLabelMap.BritListName}</th>
                <th>&nbsp;</th>
                <th>&nbsp;</th>
              </tr>
          ...
          <#list childShoppingListDatas as childShoppingListData>
              ...
              <tr>
                ...
                <td>
                  <a href="<@ofbizUrl>editShoppingList?shoppingListId=${childShoppingList.shoppingListId}</@ofbizUrl>" class="btn btn-link">${uiLabelMap.BritGoToList}</a>
                  <a href="<@ofbizUrl>addListToCart?shoppingListId=${childShoppingList.shoppingListId}</@ofbizUrl>" class="btn btn-link">${uiLabelMap.BritAddListToCart}</a>
                </td>
              </tr>
            </form>
          </#list>
...
<div class="card">
    <div class="card-header">
      <strong>${uiLabelMap.BritListItems} - ${shoppingList.listName}</strong>
            <a href="<@ofbizUrl>addListToCart?shoppingListId=${shoppingList.shoppingListId}</@ofbizUrl>" class="float-right">${uiLabelMap.BritAddListToCart}</a>
    </div>
    <div class="card-body">
      <#if shoppingListItemDatas?has_content>
                    ...
                    <td>
                      ...
                      <#if isVirtual && productVariantAssocs?has_content>
                        ...
                        <form method="post" action="<@ofbizUrl>${addToCartAction}</@ofbizUrl>" name="listreplform_${shoppingListItem.shoppingListItemSeqId}">
                          ...
                          <div class="btn-group">
                            <a href="javascript:document.listreplform_${shoppingListItem.shoppingListItemSeqId}.action='<@ofbizUrl>${addToCartAction}</@ofbizUrl>';document.listreplform_${shoppingListItem.shoppingListItemSeqId}.submit();" class="btn btn-primary mt-2">${uiLabelMap.CommonAdd}&nbsp;${shoppingListItem.quantity?string}${uiLabelMap.BritVariationToCart}</a>
                          </div>
                          <div class="btn-group">
                            <a href="javascript:document.listreplform_${shoppingListItem.shoppingListItemSeqId}.action='<@ofbizUrl>${replaceItemAction}</@ofbizUrl>';document.listreplform_${shoppingListItem.shoppingListItemSeqId}.submit();" class="btn btn-outline-secondary mt-2">${uiLabelMap.BritReplaceWithVariation}</a>
                          </div>
                        </form>
                      <#else>
                      <div class="btn-group">
        ...
        <#else>
            <h2>${uiLabelMap.BritShoppingListEmpty}.</h2>
        </#if>
    </div>
</div>
...
<div class="card">
  <div class="card-header">
    <strong>${uiLabelMap.BritShoppingListPriceTotals} - ${shoppingList.listName}</strong>
  </div>
    <div class="card-body">
      <dl class="row">
        <dt class="col-sm-2">${uiLabelMap.BritChildListTotalPrice}</dt>
        <dd class="col-sm-10 amount"><@ofbizCurrency amount=shoppingListChildTotal isoCode=currencyUomId/></dd>
        <dt class="col-sm-2">${uiLabelMap.BritListItemsTotalPrice}</dt>
...
<div class="card">
  <div class="card-header">
    ...
    <#else>
        ...
        <h2>${uiLabelMap.BritShoppingListError} ${uiLabelMap.CommonId} ${shoppingList.shoppingListId}) ${uiLabelMap.BritListDoesNotBelong}.</h2>
    </#if>
</#if>
