...
<div class="d-flex justify-content-between">
  ...
  <div class="p-2">
    ...
    <#if "Y" == (productStore.enableDigProdUpload)!>
      <a href="<@ofbizUrl>digitalproductlist</@ofbizUrl>" class="btn btn-secondary-outline">${uiLabelMap.BritDigitalProductUpload}</a>
    </#if>
  </div>
</div>
...
<div class="card">
  <div class="card-header"><strong>${uiLabelMap.BritLoyaltyPoints}</strong></div>
  <div class="card-body">
    <label>${uiLabelMap.BritYouHave} ${totalSubRemainingAmount} ${uiLabelMap.BritPointsFrom} ${totalOrders} ${uiLabelMap.BritOrderInLast} ${monthsToInclude} ${uiLabelMap.BritMonths}</label>
  </div>
</div>
</#if>
                ...
                <#assign contactMechPurposeType = partyContactMechPurpose.getRelatedOne("ContactMechPurposeType", true) />
                  <#if contactMechPurposeType??>
                    ...
                    <#if "SHIPPING_LOCATION" == contactMechPurposeType.contactMechPurposeTypeId && (profiledefs.defaultShipAddr)?default("") == contactMech.contactMechId>
                      <label>${uiLabelMap.BritIsDefault}</label>
                    <#elseif "SHIPPING_LOCATION" == contactMechPurposeType.contactMechPurposeTypeId>
                      <form name="defaultShippingAddressForm" method="post" action="<@ofbizUrl>setprofiledefault/viewprofile</@ofbizUrl>">
                        ...
                        <input type="submit" value="${uiLabelMap.BritSetDefault}" class="btn btn-outline-secondary" />
                      </form>
                    </#if>
                  <#else>
...
<form name="setdefaultshipmeth" action="<@ofbizUrl>setprofiledefault/viewprofile</@ofbizUrl>" method="post">
  ...
  <div class="card">
    <div class="card-header">
      <#if profiledefs?has_content && profiledefs.defaultShipAddr?has_content && carrierShipMethods?has_content><a href="javascript:document.setdefaultshipmeth.submit();" class="submenutextright">${uiLabelMap.BritSetDefault}</a></#if>
      <strong>${uiLabelMap.BritDefaultShipmentMethod}</strong>
    </div>
    <div class="card-body">
      <table class="table table-responsive-sm">
        ...
        <tr>
          ...
          <td>${uiLabelMap.BritDefaultShipmentMethodMsg}</td>
        </tr>
        </#if>
      </table>
    </div>
...
<div class="card">
  <div class="card-header">
  <strong>${uiLabelMap.BritFileManager}</strong>
  </div>
  <div class="card-body">
      ...
      <#else>
        <p class="card-text">${uiLabelMap.BritNoFiles}</p>
      </#if>
      <form method="post" enctype="multipart/form-data" action="<@ofbizUrl>uploadPartyContent</@ofbizUrl>">
        ...
        <label class="mr-2">${uiLabelMap.BritUploadNewFile}</label>
  ...
  <div class="card-body">
    <table class="table table-responsive-sm">
        ...
        <td>
          <#if ("CLPT_ACCEPTED" == contactListParty.statusId!)>
            <form method="post" class="form-inline" action="<@ofbizUrl>updateContactListParty</@ofbizUrl>" name="clistRejectForm${contactListParty_index}">
              ...
              <input type="submit" value="${uiLabelMap.BritUnsubscribe}" class="btn btn-outline-secondary"/>
            </form>
          <#elseif ("CLPT_PENDING" == contactListParty.statusId!)>
            <form method="post" class="form-inline" action="<@ofbizUrl>updateContactListParty</@ofbizUrl>" name="clistAcceptForm${contactListParty_index}">
              ...
              <input type="submit" value="${uiLabelMap.BritVerifySubscription}" class="btn btn-outline-secondary"/>
              </div>
            </form>
          <#elseif ("CLPT_REJECTED" == contactListParty.statusId!)>
            <form method="post" class="form-inline" action="<@ofbizUrl>updateContactListParty</@ofbizUrl>" name="clistPendForm${contactListParty_index}">
              ...
              <input type="submit" value="${uiLabelMap.BritSubscribe}" class="btn btn-outline-secondary"/>
            </form>
          </#if>
        </td>
      </tr>
      ...
      <form method="post" class="form-inline" action="<@ofbizUrl>createContactListParty</@ofbizUrl>" name="clistPendingForm">
        <input type="hidden" name="partyId" value="${party.partyId}"/>
        <label class="mr-2">${uiLabelMap.BritNewListSubscription}: </label>
        <input type="hidden" name="statusId" value="CLPT_PENDING"/>
    ...
    <label class="mt-2">${uiLabelMap.BritListNote}</label>
  </div>
</div>
...
<div class="card">
  <div class="card-header">
    <strong>${uiLabelMap.BritSurveys}</strong>
  </div>
  <div class="card-body">
    <table class="table table-responsive-sm">
      <#list surveys as surveyAppl>
        ...
        <tr>
          ...
          <td>
            ...
            <#if (responses < 1)>${uiLabelMap.BritNotCompleted}<#else>${uiLabelMap.BritCompleted}</#if>
          </td>
          <#if (responses == 0 || "Y"  == survey.allowMultiple?default("N"))>
            <#assign surveyLabel = uiLabelMap.BritTakeSurvey />
            <#if (responses > 0 && "Y"  == survey.allowUpdate?default("N"))>
              <#assign surveyLabel = uiLabelMap.BritUpdateSurvey />
            </#if>
...
${screens.render("component://brit/widget/CustomerScreens.xml#messagelist-include")}

${screens.render("component://brit/widget/CustomerScreens.xml#FinAccountList-include")}

<#-- Serialized Inventory Summary -->
${screens.render('component://brit/widget/CustomerScreens.xml#SerializedInventorySummary')}

<#-- Subscription Summary -->
${screens.render('component://brit/widget/CustomerScreens.xml#SubscriptionSummary')}

<#-- Reviews -->
${screens.render('component://brit/widget/CustomerScreens.xml#showProductReviews')}
