 ...
 * under the License.
 *******************************************************************************/
package org.apache.ofbiz.brit.misc;
...
public class ThirdPartyEvents {
    ...
    public static String updateAssociatedDistributor(HttpServletRequest request, HttpServletResponse response) {
        ...
        java.net.URL britPropertiesUrl = null;

        try {
            britPropertiesUrl = request.getServletContext().getResource("/WEB-INF/brit.properties");
        } catch (java.net.MalformedURLException e) {
            Debug.logWarning(e, MODULE);
        }

        String store = UtilProperties.getPropertyValue(britPropertiesUrl, "distributor.store.customer");

        if (store == null || store.toUpperCase().startsWith("N")) {
            return "success";
        }

        String storeOnClick = UtilProperties.getPropertyValue(britPropertiesUrl, "distributor.store.onclick");
    ...
    public static String updateAssociatedAffiliate(HttpServletRequest request, HttpServletResponse response) {
        ...
        String store = UtilProperties.getPropertyValue(britPropertiesUrl, "affiliate.store.customer");

        if (store == null || store.toUpperCase().startsWith("N")) {
            return "success";
        }
        String storeOnClick = UtilProperties.getPropertyValue(britPropertiesUrl, "affiliate.store.onclick");
